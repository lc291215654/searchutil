...
import com.bigdata.gather.AsyncGather;
import com.bigdata.gather.CommonSpider;
import com.bigdata.gather.QuartzManager;
import com.bigdata.gather.WebpageSpiderJob;
...
public class CommonsSpiderService extends AsyncGatherService {
    ...
    private void validateSpiderInfo(SpiderInfo spiderInfo) {
        Preconditions.checkArgument(spiderInfo.getStartURL().size() > 0,
                "起始地址列表不能为空");
        Preconditions.checkArgument(StringUtils.isNotBlank(spiderInfo.getDomain()),
                "domain不可为空");
        Preconditions.checkArgument(!spiderInfo.getDomain().contains("/"),
                "域名不能包含/");
        Preconditions.checkArgument(spiderInfo.getThread() > 0,
                "线程数必须大于0");
        Preconditions.checkArgument(StringUtils.isNotBlank(spiderInfo.getSiteName()),
                "网站名称不可为空");
        Preconditions.checkArgument(spiderInfo.getTimeout() > 1000,
                "超时时间必须大于1秒");
